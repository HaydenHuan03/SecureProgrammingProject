...
export function AdminPanel() {
  ...
  function handleDelete(userId: string) {
    doDelete(userId, { onSuccess: () => setDeleteConfirm(null) });
  }
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          ...
          <div style={styles.headerRight}>
            <span style={styles.badge}>Logged in as <strong>{me?.username}</strong></span>
            <button
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.user_id} style={styles.tr}>
                    ...
                    <td style={styles.td}>
                      <span style={{ ...styles.roleBadge, background: u.role === "admin" ? "#7c3aed" : "#059669" }}>
                        {u.role}
                      </span>
                    </td>
          ...
          <div style={styles.modal}>
            ...
            <form onSubmit={handleUpdate} style={styles.modalForm}>
              <label style={styles.label}>
                Username
                <input style={styles.input} value={editUsername} onChange={(e) => setEditUsername(e.target.value)} required />
              </label>
              <label style={styles.label}>
                Email
              ...
              <label style={styles.label}>
                New Password <span style={{ color: "#6b7280", fontWeight: 400 }}>(leave blank to keep)</span>
                <input style={styles.input} type="password" value={editPassword} onChange={(e) => setEditPassword(e.target.value)} />
              </label>
              <label style={styles.label}>
                Role
                <select style={styles.input} value={editRole} onChange={(e) => setEditRole(e.target.value as Role)}>
                  <option value="user">user</option>
                  <option value="admin">admin</option>
                </select>
              </label>
        ...
        <div style={styles.overlay}>
          <div style={styles.modal}>
            ...
            <div style={styles.modalActions}>
              ...
              <button style={styles.deleteBtn} onClick={() => handleDelete(deleteConfirm)}>Delete</button>
            </div>
          </div>
        </div>
      )}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  createForm: { display: "flex", gap: "0.75rem", flexWrap: "wrap" },
  ...
  th: { textAlign: "left", padding: "0.625rem 0.75rem", borderBottom: "2px solid #e5e7eb", fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", color: "#6b7280" },
  tr: { borderBottom: "1px solid #f3f4f6" },
  td: { padding: "0.625rem 0.75rem", fontSize: "0.875rem" },
  roleBadge: { display: "inline-block", padding: "2px 8px", borderRadius: "9999px", color: "#fff", fontSize: "0.75rem", fontWeight: 500 },
  ...
  modal: { background: "#fff", borderRadius: "8px", padding: "1.5rem", width: "360px", boxShadow: "0 4px 16px rgba(0,0,0,0.2)" },
